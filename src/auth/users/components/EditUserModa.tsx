...
export const EditUserModal = ({
  ...
  const [role, setRole] = useState(userValue.role);
  const [address, setAddress] = useState(userValue.address);
  ...
  const handleSubmit = async (e: FormEvent) => {
    try {
      const { data } = await axiosClient.put(`/auth/${userValue.id}`, {
        name,
        email,
        password,
        role,
      ...
      <form onSubmit={handleSubmit}>
        <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-lg ">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-medium text-black">Editar Usuario</h3>
            <button
