...
export const EditProductModal = ({ onClose, productValue }: Props) => {
  ...
  const [categoryId, setCategoryId] = useState(productValue.categoryId);
  const [stock, setStock] = useState(productValue.stock);
  const [makerId, setMakerId] = useState(productValue.makerId);
  ...
  const handleSubmit = async (e: FormEvent) => {
    try {
      const { data } = await axiosClient.put(`/product/${productValue.id}`, {
        name,
        price,
        description,
        categoryId,
